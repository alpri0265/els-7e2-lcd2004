...
void LiquidCrystalRus::send(uint8_t value, uint8_t mode) {
  ...
  if (_displayfunction & LCD_8BITMODE) {
    ...
    // STM32 @168MHz can violate LCD2004 timing margins on 3.3V logic.
    // Make inter-nibble delay much longer than the HD44780 minimum.
...
void LiquidCrystalRus::pulseEnable() {
  digitalWrite(_enable_pin, LOW);
  // Make EN low time and pulse width significantly longer.
  delayMicroseconds(250);    
  digitalWrite(_enable_pin, HIGH);
  delayMicroseconds(2500);    // enable pulse must be >450ns (greatly increased)
  digitalWrite(_enable_pin, LOW);
  delayMicroseconds(25000);   // commands need > 37us to settle (greatly increased)
}
...
void LiquidCrystalRus::writeNbits(uint8_t value, uint8_t n) {
  for (int i = 0; i < n; i++) {
    ...
    // Extra settling time per data line update.
