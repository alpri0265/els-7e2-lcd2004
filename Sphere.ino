  ...
  if (Motor_X_Pos == Limit_Pos_Rear && Motor_Z_Pos == Limit_Pos_Right && Step_X_flag == OFF)
   { 
    ...
    w = w + Excess_Lag; 
      ...
      {  _delay_ms(250);
         ...
         {
            if (Pass_Nr <= Pass_Total_Sphr / 2) Infeed_ValueZ = (MOTOR_Z_STEP_PER_REV * (Cutting_Width * Pass_Nr) / SCREW_Z +0.5) * McSTEP_Z;
            else                                Infeed_ValueZ = (MOTOR_Z_STEP_PER_REV * ((Cutting_Width * (Pass_Nr - 1) + Cutter_Width)) / SCREW_Z +0.5) * McSTEP_Z;
         }
   ...
   {  _delay_ms(250);
      if (Pass_Nr <= Pass_Total_Sphr + 1)
      {
         ...
         {   
            Infeed_ValueX = (Sph_R_mm - (sqrt((Sph_R_mm * Sph_R_mm) - (Sph_R_mm - Cutting_Width * Pass_Nr)*(Sph_R_mm - Cutting_Width * Pass_Nr)))) * MOTOR_X_STEP_PER_REV / SCREW_X * McSTEP_X;
         } 
         
         else if (Pass_Nr > Pass_Total_Sphr / 2 )                     // после экватора
         {  
            Infeed_ValueX = (Sph_R_mm - (sqrt((Sph_R_mm * Sph_R_mm) - (Cutting_Width * (Pass_Nr - (Pass_Total_Sphr / 2 + 1)))*(Cutting_Width * (Pass_Nr - (Pass_Total_Sphr / 2 + 1)))))) * MOTOR_X_STEP_PER_REV / SCREW_X * McSTEP_X;
            if (Infeed_ValueX >= SphBar) Infeed_ValueX = SphBar; // оставить ножку  
         }
         X_pos = X_pos - Excess_Lag;   
         Limit_Pos_Front = (Null_X_Pos + Infeed_ValueX + w - Excess_Lag);
   ...
   {  _delay_ms(250);
      ...
      Limit_Pos_Rear = (Null_X_Pos - REBOUND_X + w - Excess_Lag);
      Pass_Nr++;
